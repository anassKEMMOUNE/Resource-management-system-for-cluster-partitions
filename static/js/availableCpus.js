...
function numberOfCpus() {
    ...
    const jsonFilePath2 = "static/json/result2.json";
    var jsonObject = {};
    var output = {};
    ...
    return Promise.all([fetchJsonFile(jsonFilePath2), fetchJsonFile(jsonFilePath)])
        .then(([data2, data]) => {
            ...
            for (partition in jsonObject) {
                ...
                output[partition].TotalCPUs = parseInt(numberOfCpu);
                output[partition].CPUAlloc = 0;

                nodelist.forEach(function (node) {
                    output[partition].CPUAlloc += parseInt(window.jsonObject2[node].CPUAlloc);
                });
            }
